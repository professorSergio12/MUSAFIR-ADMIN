...
type FoodFormData = {
  ...
  image: string | File;
  imagePreview: string;
  foodOptions: { name: string; surcharge: number }[];
};
...
const SingleFoodOption = ({ onClose }: SingleFoodOptionProps) => {
  ...
  const handleCancel = () => {
    ...
    if (singleFoodData) {
      ...
      if (typeof formData.image !== 'string' && formData.imagePreview) {
  ...
  const handleSave = async () => {
    ...
    // Send new file if selected; otherwise backend keeps existing image
  ...
  const handleRemoveImage = () => {
    if (formData.imagePreview && typeof formData.image !== 'string') {
      URL.revokeObjectURL(formData.imagePreview);
    }
    setFormData(prev => ({
      ...prev,
  ...
  // Image URL for display: when editing and a File is selected use preview, else use string URL
  const displayImageSrc =
    isEditing
      ? (typeof formData.image === "string" ? formData.image : formData.imagePreview)
      : (singleFoodData?.image ?? "");
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex-1">
                                  <p className="text-xs text-slate-600">
                                    {typeof formData.image !== 'string' ? formData.image.name : 'Existing Image'}
                                  </p>
